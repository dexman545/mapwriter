...
public class MapView {
    ...
    public void setDimension(DimensionType dimension) {
        ...
        if (dimension != this.dimension) {
            if ((this.dimension != DimensionType.THE_NETHER) && (dimension == DimensionType.THE_NETHER)) {
                scale = 0.125;
            } else if ((this.dimension == DimensionType.THE_NETHER) && (dimension != DimensionType.THE_NETHER)) {
                scale = 8.0;
            }
    ...
    public void nextDimension(List<DimensionType> dimensionList, int n) {
        int i = dimensionList.indexOf(this.dimension);
        i = Math.max(0, i);
        int size = dimensionList.size();
        DimensionType dimension = dimensionList.get((i + size + n) % size);
    ...
    public double getDimensionScaling(DimensionType playerDimension) {
        double scale;
        if ((this.dimension != DimensionType.THE_NETHER) && (playerDimension == DimensionType.THE_NETHER)) {
            scale = 8.0;
        } else if ((this.dimension == DimensionType.THE_NETHER) && (playerDimension != DimensionType.THE_NETHER)) {
            scale = 0.125;
        } else {
            scale = 1.0;
        }
